...
def COUNT(teksti):
   ...
   for i in teksti:
      if(i=='b' or i=='c' or i=='d' or i=='f' or i=='g' or i=='h' or i=='j' 
         ...
         or i=='L' or i=='M' or i=='N' or i=='P' or i=='Q' or i=='R'
         or i=='S' or i=='T' or i=='V' or i=='W' or i=='X' or i=='Z' ):
   ...
   return("Në këtë tekst janë " + str(bashketingellore) + " bashkëtingellore dhe " + str(zanore) +" zanore")
    #Metoda REVERSE
def REVERSE(teksti):
    fjaliaReverse=""
    for i in teksti:
...
def GAME():
   ...
   for i in range(36):
      ...
      if f!=5:
        ...
        break
   randomN.sort()   
   str1 = ','.join(str(e) for e in randomN)
   return(str1)
    #Metoda CONVERT
def CONVERT(opsioni, numri):
    if(opsioni.lower().strip()== 'cmtofeets'):
        ...
        vlera = float(numri) * 0.621371
    elif(opsioni.lower().strip() == 'miletokm'):
        vlera = float(numri) * 1.609
    else:
        vlera = "Ky konvertim nuk mund te behet! Jepni nje nga opsionet e mundshme te parapara per konvertim ."
    return vlera
    #Metoda GCF
def GCF(nr1,nr2):
    if nr1%nr2==0:
...
def Pergjigjja(hyrja, address):
        try:
            data  = hyrja.decode("utf-8") 
        except socket.error:
...
while True:
    ...
    print("Serveri është lidhur me klientin me IpAdresë %s, në portin %s" % address + " ka bërë kërkesën : " + data.decode("utf-8"))
